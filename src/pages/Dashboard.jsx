      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-[2rem] border border-surface-200 shadow-sm overflow-hidden flex flex-col">
          <div className="px-8 py-6 border-b border-surface-100 bg-red-50/10">
            ...
            <p className="text-xs text-surface-400 font-bold mt-0.5">{t('Below threshold of 10 units')}</p>
          </div>
          ...
          <div className="p-4 flex-1">
            {lowStockProducts.length > 0 ? (
                ...
                {lowStockProducts.map(p => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[10px] text-surface-400 font-bold tracking-tighter">{t('STOCK')}: {p.stockQuantity} {t('UNITS')}</p>
                    </div>
                    <div className="text-red-600 font-black text-xs animate-pulse">{t('Restock')} →</div>
                  </Link>
                ))}
              </div>
            ) : (
              <div className="h-full flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-surface-500 font-bold text-sm px-6">{t('Your inventory levels are looking healthy!')}</p>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-surface-900 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden group">
            ...
            <h3 className="text-2xl font-black mb-2">{t('Smart Recommendations')}</h3>
            <p className="text-white/60 font-medium mb-8 max-w-md italic">"{t('Based on your assessment, focuses on Sales & Inventory automation to increase efficiency by up to 30%')}"</p>
            
            <div className="flex flex-wrap gap-4">
              <Link to="/dashboard/sales" className="px-6 py-3 rounded-xl bg-white text-surface-900 text-xs font-black hover:bg-primary-50 transition-all">{t('Setup Invoicing')}</Link>
              <Link to="/dashboard/inventory" className="px-6 py-3 rounded-xl bg-white/10 text-white text-xs font-black hover:bg-white/20 transition-all border border-white/10">{t('Manage Stock')}</Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="bg-white rounded-[2.5rem] p-8 border border-surface-200 shadow-sm">
              ...
              <h4 className="font-black text-surface-900 mb-2">{t('Build Relations')}</h4>
              <p className="text-surface-500 text-xs font-medium leading-relaxed mb-6">{t('Track customer lifetime value and purchase history to provide personalized service')}</p>
              <Link to="/dashboard/crm" className="text-primary-600 font-bold text-xs hover:underline decoration-2">{t('Manage Customers')} →</Link>
            </div>
            <div className="bg-white rounded-[2.5rem] p-8 border border-surface-200 shadow-sm">
